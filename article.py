...
def generate_html(output_root: Path):
    """Generate HTML from articles/ and images_out/ in the project root"""
    print(f"Processing articles in: articles/ and images in: images_out/")
...
"""
    ...
    # Copy all images from images_out/ to assets/images/
    images_source = Path('images_out')
    ...
    if articles_folder.exists():
        for md_file in articles_folder.glob('*.md'):
            ...
            # Fix image paths to point to images_out with correct relative path
            text = re.sub(r'!\[(.*?)\]\(\.\./images_out/(.*?)\)',
                         lambda m: f"![{m.group(1)}](../../images_out/{m.group(2)})",
                         text)
    ...
    for art in all_articles:
        if art['image']:
            # Always use assets/images/... for card images
            img_path = art['image']
            # Remove any leading ../ or ./
            img_path = re.sub(r'^\.\./', '', img_path)
            img_path = re.sub(r'^\./', '', img_path)
            # Remove any leading images_out/ and replace with assets/images/
            img_path = re.sub(r'^images_out/', 'assets/images/', img_path)
            card_img_src = img_path
            img_html = f'<img src="{card_img_src}" alt="{art["title"]}" class="w-full h-48 object-cover rounded-t-lg">'
        else:
